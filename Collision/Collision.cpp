...
void CollisionEdit(const std::vector<std::vector<WorldTransform>>& mapchip, const std::vector<std::vector<uint32_t>>& type, WorldTransform* transform, Vector3* velocity)
{
	...
	if (velocity->x != 0 || velocity->y != 0) {
		...
		int y = kMaxHeightBlockNum - static_cast<int>((transform->worldPos_.y + transform->scale_.y) / (mapchip[0][0].scale_.y * 2));
		int x = static_cast<int>((transform->worldPos_.x - transform->scale_.x) / (mapchip[0][0].scale_.x * 2));
