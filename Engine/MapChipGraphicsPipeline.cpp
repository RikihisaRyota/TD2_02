...
void MapChipGraphicsPipeline::InitializeGraphicsPipeline() {
	CreateState();
	CreatePSO();
}

void MapChipGraphicsPipeline::CreateState() {
	CreateRootsignature();
	CreateInputLayout();
	CreateBlendState();
...
void MapChipGraphicsPipeline::CreateRootsignature() {
	HRESULT hr = S_FALSE;
	// Rootsignature設定
	...
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::TEXTURE)].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;// PixelShaderで使う
...
void MapChipGraphicsPipeline::CreateInputLayout() {
	//InputLayout
	inputLayout_[0] =
	{// xyz座標(1行で書いたほうが見やすい)
...
void MapChipGraphicsPipeline::CreateBlendState() {
	//全ての色要素を書き込む
	blendDesc_.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
}

void MapChipGraphicsPipeline::CreateRasiterzerState() {
	//裏面（時計回り）を表示しない
...
void MapChipGraphicsPipeline::CreateShaderCompile() {
	//Shaderをコンパイルする
	vertexShaderBlob_ = ShaderCompiler::Compile(
		L"Resources/Shaders/MapChip.VS.hlsl",
		L"vs_6_0");
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = ShaderCompiler::Compile(
		L"Resources/Shaders/MapChip.PS.hlsl",
		L"ps_6_0");
	assert(pixelShaderBlob_ != nullptr);
}

void MapChipGraphicsPipeline::CreateDepthStencil() {
	// Depthの機能を有効化する
...
void MapChipGraphicsPipeline::CreatePSO() {
	HRESULT hr = S_FALSE;
	//PSO生成
